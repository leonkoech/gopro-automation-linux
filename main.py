...
def start_recording(gopro_id):
    """Start recording on a specific GoPro and save to Jetson"""
    ...
    with recording_lock:
        ...
        try:
            ...
            cmd = [
                ...
                '--download'  # Automatically download to Jetson
            ]

            # Start gopro-video in a separate thread so Flask doesn't block
...
def stop_recording(gopro_id):
    """Stop recording on a specific GoPro and ensure video is saved to Jetson"""
    global recording_processes
    ...
    try:
        # Gracefully stop the GoPro recording (triggers beep)
        os.killpg(os.getpgid(process.pid), signal.SIGINT)

        # Wait for gopro-video to finish downloading the video
        try:
            process.wait(timeout=300)  # 5 min max for download
        ...
        success = video_exists and file_size > 0
