    ...
    def _sweep(self) -> None:
        ...
        for doc in query.stream():
            try:
                ...
                # Evaluated here on the streamed snapshot rather than inside the
                # transaction: ``active_sessions`` is fetched once per sweep, so
                # re-reading it under the transaction would be no fresher. The
                # transaction's job is the ``endedAt`` race (UBA-304).
                events = _sorted_events(game.get('logs') or [])
                last_activity = _game_last_activity(game, events)
